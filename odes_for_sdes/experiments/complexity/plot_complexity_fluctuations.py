# -*- coding: utf-8 -*-
"""
Created on Fri May 29 11:49:18 2020
...
reps = 20
thresholds = {1:10**(-2), 2:5*10**(-3), 3:10**(-3),4:5*10**(-4) }
...
for thr in range(1,5):
    ...
    for repi in range(reps):
        ...
        try:     
            D = joblib.load(filename=foldername+'Comp_complexity_particle_number_MEAN_FLUCTUATIONS_OU%d_trial_%d'%(thr,repi))
            N_det[thr-1,repi] = (D['N_star'])[repi]
        except FileNotFoundError:
            N_det[thr-1,repi] = np.nan
            print('run id: %d, trial: %d' %(thr,repi))
...
for thr in range(1,5):
    for repi in range(reps):
        if thr==3 and repi>=5:
                nss = [0,0,0,0,0,35020,29900,54810,57020,34620,39910,30000,45110,35930,84010,29930,48670,30400,31410,68500]
                N_sto[thr-1,repi] = nss[repi]
        else:
            try:     
                D = joblib.load(filename=foldername+'Comp_complexity_particle_numberStoch_MEAN_FLUCTUATIONS_OU%d_trial_%d'%(thr,repi))
                N_sto[thr-1,repi] = (D['N_star'])[repi]
            except FileNotFoundError:
                N_sto[thr-1,repi] = np.nan
                print('run id: %d, trial: %d' %(thr,repi))
...
for thr in range(1,5):
    ...
    df_list[max_indx].dropna(subset=[0])
    max_indx += 1
df_det=pd.concat(df_list)
df_det.dropna(subset=[0])
...
plt.savefig('Complexitybfl.pdf',  bbox_inches = 'tight', pad_inches = 0.1)     
plt.savefig('Complexitybfl.png',  bbox_inches = 'tight', pad_inches = 0.1)  
