# -*- coding: utf-8 -*-
"""
Created on Tue Apr 14 23:54:31 2020
...
for ss,N in ite:
    if N >= 0 and N<=7000:
        ...
        for i in range(0,trials):
            ...
            for ti,t in enumerate(timegrid):
                if ti==0: 
                    ...
                    D[:,:,ti] = D[:,:,ti-1] + h* f(D[:,:,ti-1],t)+  (g)*np.array([np.random.normal(loc = 0.0, scale = np.sqrt(h),size=(dim,N))])#.reshape(2,)
            ...
            filenm = 'N_vs_dims_for_OU_with_KL_opt_N_for_STOCHASTIC_for_dim_%d_FEWERNs_UPDATED_05_N_%d'%(dim,N)
        ...
        #joblib.dump(Dic, filename= save_file+filenm+'FIN')
