...
#Didi['C_ts'] = C_ts
#Didi['Ns'] = Ns
#joblib.dump(Didi,filename='Data_for_Dims_OUs_M200')
#            
#%% Load deterministic data
            
Didi = joblib.load('Data_for_Dims_OUs_M200')    
klls = Didi['klls'] 
means = Didi['means'] 
covs = Didi['covs']
m_ts = Didi['m_ts'] 
...
#    for i in range(13):
#        plt.subplot(3,5,i+1) 
#        plt.plot(klls[di][:,:,i]),plt.yscale('log')    
#        plt.title('N = %d'%Ns[i])
#        if i%5==0:
#            plt.ylabel('KL')
#    plt.subplots_adjust(wspace = 0.35,hspace=0.35)
#    plt.savefig('multiOU_kls_vs_time_dim%d'%di)
...
#              data=df_klend, dodge=True, alpha=.1, zorder=1,ax=ax)
...
g1=sns.pointplot(x='N', y=u'$KL^{T}$', hue='D',
              data=sdf_klmiddle, dodge=.932, palette='mycoloryellow',# palette=cpal,#palette=sns.cubehelix_palette(8, start=2, rot=0, dark=0, light=.95, reverse=True),#color ='#464646',
...
ax.legend(handles[4:9], labels2[4:9], title=None,
...
g1=sns.pointplot(x='N', y=u'$KL^{T}$', hue='D',
              data=sdf_klend, dodge=.832, palette='mycoloryellow',# palette=cpal,#palette=sns.cubehelix_palette(8, start=2, rot=0, dark=0, light=.95, reverse=True),#color ='#464646',
              markers=["o",'d','s','v','P'], scale=1.4, ci='sd',ax=ax,linewidth=8,linestyles='--',edgecolor='gray',errwidth=3, join=True,alpha=0.6,zorder=0)
plt.setp(g1.lines, alpha=.5,linewidth=4) 
plt.setp(g1.collections,edgecolor=col_gr,linewidth=2,zorder=50)
g2=sns.pointplot(x='N', y=u'$KL^{T}$', hue='D',
...
g1=sns.pointplot(x='N', y='x', hue='D',
              data=sdf_meanend, dodge=.732,   palette='mycoloryellow',#  palette=cpal,#palette=sns.cubehelix_palette(8, start=2, rot=0, dark=0, light=.95, reverse=True),#color ='#464646',
...
normcovs = dict()
for dim in dims:
    normcovs[dim] = np.zeros((20,int(tim[dim]),Ns.size))
    for ti in range(int(tim[dim])):
        for tr in range(20):
            for ni,N in enumerate(Ns):

                normcovs[dim][tr,ti,ni] =np.linalg.norm(covs[dim][:,:,ti,tr,ni]-C_ts[dim][ti,:,:].T,ord='fro')            
...
for di in dims:
    ...
    for ni,N in enumerate(Ns):
        plt.subplot(5,3,ni+1)
        plt.plot(normcovs[dim][:,:,ni])
...
# Show the conditional means
g1=sns.pointplot(x='N', y='x', hue='D',
              data=sdf_covsend, dodge=.732,   palette='mycoloryellow',#  palette=cpal,#palette=sns.cubehelix_palette(8, start=2, rot=0, dark=0, light=.95, reverse=True),#color ='#464646',
...
l1 = ax.legend([handles[-1],handles[2]], ['D','S'], title=None,
          handletextpad=0, columnspacing=0.1,
           ncol=2, frameon=False,fontsize=16,bbox_to_anchor=(0.35, 0.86))
