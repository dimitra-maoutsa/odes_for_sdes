...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 24 16:51:47 2020
...
#x0 = np.array([0,0,0,0])
sigm = 0.5
g = lambda x: np.sqrt(x) #* np.sin( x)#np.array([0.5,2,0.5,1])
#potential
...
#f = lambda x,t: np.array([ -x[0]*x[0]*x[0]*x[0]*x[0]+x[0]*x[0]*x[0]-0.1*x[0]]) #anotherdouble well 
f = lambda x,t: np.array([ -4*x[0]])
rho = 28
sigm = 10
beta = 8/3.
...
Ns = 2500 # number of stochastic particles
N = 500  #number of particles
#diffission coefficient/fucntion

gii = lambda x,t:g(x)#np.multiply(np.ones(dim),g)
xs = np.zeros((dim,Ns))
for ii in range(dim):
    xs[ii] = np.random.normal(loc=x0[ii], scale=0.25,size=Ns)
...
def f_seperate(x,t):#plain GP prior
    ...
    lnthsc = 3*np.std(x,axis=1)
    #print(lnthsc)
    for ii in range(dimi):        
        gpsi[ii,:]= score_function_multid_seperate(x.T,Zxx.T,'None',0,C=0.001,which=1,l=lnthsc,which_dim=ii+1)[0] #C=0.1 !!!!!   
    gi_term = gii(x,t)**2#np.tile(gii(x,t)**2 ,(N,1)).T
    
#    print(np.multiply( gi_term, gpsi).shape)
#    print(( 2* sigm**2 *x).shape )
    return (f(x,t)-0.5* np.multiply( gi_term, gpsi) )  #the last term should always be replased by the gradient of gii(x,t)**2
...
for ti,t in enumerate(timegrid):
    ...
    if ti==0: 
        ...
        for i in range(Ns):
            ...
            F[:,i,ti] = F[:,i,ti-1] + h* f(F[:,i,ti-1],t) + np.multiply(gii(F[:,i,ti-1],t),np.array([np.random.normal(loc = 0.0, scale = np.sqrt(h),size=dim) ]).reshape(dim,))
...
#            print(ti)
    for ti in range(0,10): 
        plt.subplot(4,3,ii)
        sns.distplot(F[:,ti],100,hist=True, kde=True, label='S',color=col_gr)
        sns.distplot(G[:,ti],100,hist=True, kde=True, label='D',color=col_ro)
...
#    F = Dict['F']       
    ...
    for di in range(dim):
        ...
        plt.plot(timegrid, np.mean(F[di,:,:],axis=0),lw=2.5,label='S',color=col_gr)
        ...
        plt.plot(timegrid, np.std(F[di,:,:],axis=0),lw=2.5,label='S',color=col_gr)
        ...
        plt.plot(timegrid, skew(F[di,:,:],axis=0),lw=2.5,label='S',color=col_gr)
        ...
        plt.plot(timegrid, kurtosis(F[di,:,:],axis=0),lw=2.5,label='S',color=col_gr)
...
#        ax_lims[di,1] += padi
    ...
    for ti in range(0,timegrid.size-1,50):
